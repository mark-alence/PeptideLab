...
// --- Listen for drag start from palette (desktop only) ---
...
// --- Listen for delete/undo from ActionBar (mobile) ---
...
function tryPlace(letter) {
  ...
  if (entry) {
    ...
    // On mobile, deselect after placing so user must pick again
    if (isMobile) {
      selectedAA = null;
      GameEvents.emit('selectionChanged', { letter: null });
    }
...
export function updateInput() {
  ...
  // On mobile: only show highlight when pointer is actively on canvas
  if (isMobile && !canvasPointerDown && !isDragging) {
    hideHighlight();
    return;
  }
  ...
  updateHover();
  const cell = getHoveredCell();
  const active = paletteDrag || isDragging || selectedAA;

  if (!cell || !active) {
    if (isMobile) hideHighlight();
    return;
  }
